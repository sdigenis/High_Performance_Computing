  ...
  *h_OutputCPU, *d_OutputGPU;
  ...
  if(imageW > 8192){
    for(i = 0; i < times*times; i++){
      cudaHostAlloc((void**)&input_big[i], dimW * ((MAXCAP/ imageH) + 2 * filter_radius) * sizeof(mine_t), cudaHostAllocDefault);
      //input_big[i] = (mine_t*)malloc( dimW * ((MAXCAP/ imageH) + 2 * filter_radius) * sizeof(mine_t)); 
      cudaHostAlloc((void**)&output_big[i], dimW * ((MAXCAP/ imageH) + 2 * filter_radius) * sizeof(mine_t), cudaHostAllocDefault);
  ...
  if(imageH <= 8192){
    ...
    printf("GPU computation...\n");
    for(i = 0; i < times * times; i++){
        cudaMemcpy(d_Input, input_big[i], dimW * ((MAXCAP/ imageH) + 2 * filter_radius)* sizeof(mine_t), cudaMemcpyHostToDevice);
        ...
        cudaMemcpy(output_big[i], d_OutputGPU, dimW * ((MAXCAP/ imageH) + 2 * filter_radius) * sizeof(mine_t), cudaMemcpyDeviceToHost);
        //printf("i: %d\n", i);
        error = cudaGetLastError();
        if(error != cudaSuccess)
        {
